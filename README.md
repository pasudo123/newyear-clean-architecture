...
---
## 22장 클린 아키텍처
<img width="500" src="images/clean_architecture.png" />

### 의존성 규칙
* 안으로 들어갈수록 고수준의 소프트웨어가 된다.
* 바깥쪽 원은 메커니즘, 안쪽 원은 정책이다.
  * 소스코드 의존성은 반드시 안쪽으로, 고수준의 정책으로 향해야 한다.

### 원은 네 개여야만 하나?
* 그저 개념을 설명해주기 위한 예시일 뿐. 네 개보다 더 많은 원이 필요할 수도 있다.
* 다만 어떤 경우에도 의존성 규칙은 적용된다.
  * `소스 코드 의존성은 항상 안쪽을 향한다.`
  * 안쪽으로 이동할수록 추상화와 정책의 수준이 높아진다.

---
## 25장 계층과 경계 (p.236 : 게임을 예시로 컴포넌트의 발전과 경계에 대해서 설명해줌, 다시 읽어봐도 무방)
* 시스템이 세 가지 컴포넌트 (UI, 업무규칙, 데이터베이스) 로만 구성되어있지 않다.
  * 대다수의 시스템에서 컴포넌트의 개수는 이보다 훨씬 많다.
* 아키텍처의 경계가 어디에나 존재한다.
  * 다만 추상화가 필요하리라고 미리 예측해서는 안된다. (p.243)
...
### 야옹이 문제 (p.254 : 책을 좀 더 살피자)
* 서비스 다이어그램을 살피면, 이 기능을 구현하려면 이들 서비스 중 어디를 변경해야 하는가?
  * 전부다 변경해야 한다.
  * 의심의 여지없이 야옹이 배달 기능을 추가하려면 개바로가 배포 전략을 매우 신중하게 조정해야 한다.
  * 다시 말해 이 서비스들은 모두 결합되어 있어어 독립적으로 개발하고, 배포하거나, 유지될 수 없다.
* `횡단관심사가 지닌 문제`
  * 기능적 부냏는 새로운 기능이 기능적 행위를 횡단하는 상황에 매우 취약하다 : `책의 내용을 다시 살핀다.`
...
* 시스템에 가한 간단한 변경이 대량의 테스트 실패로 이어진다는 사실을 알게되면, 개발자는 그러한 변경을 하지 않을 것이다. -> 최악의 상황
* GUI 는 변동성이 크다. GUI 로 시스템을 조작하는 테스트 스위트는 분명 깨지기 쉽다.
  * GUI 를 사용하지 않고, 업무 규칙을 테스트할 수 있어야 한다.

---
## 30장 데이터베이스는 세부사항이다
### 데이터베이스 시스템은 왜 이렇게 널리 사용되는가? (p.291)
* 디스크는 10여개의 플래터로 구성되었다.
* 디스크에서 특정 바이트를 읽으려면 먼저 헤드를 적절한 트랙으로 옮기고, 
* 디스크가 돌면서 헤드 위치에 적절한 섹터가 올 때 까지 기다린 후,
* 해당 섹터에서 4K 모두를 RAM 으로 읽어들여야 한다.
* 그런 후 해당 RAM 버퍼의 색인을 찾아서 필요한 바이드를 가져왔다.
...
## 31장 웹은 세부사항이다.
### 요약
* 웹은 GUI 다. 따라서 웹은 세부사항이다. 그리고 아키텍트라면 이러한 세부사항을 핵심 업무 로직에서 분리된 경계 바깥에서 두어야 한다.
